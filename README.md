...
- **BING每日壁纸**: 接入Bing每日壁纸，每一次刷新都是新的风景。
- **窗口体验优化**: 设置窗口现支持拖动，交互更灵活。
- **视觉风格升级**: 变更了壁纸样式，优化组件显示错误问题，并添加了鼠标悬停效果。
- **开放中心**: 重新划分区域，新增开放中心，支持自定义CSS，打造专属个性化页面。
- **Webhook支持**: 添加了Webhook接口支持，扩展性更强。
- **浏览器插件**: 添加了浏览器插件下载路径，方便用户获取。
